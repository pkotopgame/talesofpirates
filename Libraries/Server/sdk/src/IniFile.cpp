	...
	try{
		...
		while( !feof( l_file ) )
		{
			...
			if((l_str1	=ltrim(l_szbuf))[0]=='['){
				...
				if(!l_str2){
					THROW_EXCP(excpFile, "INI文件格式错误:" + std::to_string(l_lino) + "行");
				}
				*l_str2	=0;
...
IniSection & IniFile::operator[](int i){
	...
	if(i>=0 && i<m_sectcount){
		...
		THROW_EXCP(excpArr,"IniFile操作下标:" + std::to_string(i) + "越界");
	}
	return *l_is;
}
...
IniItem & IniSection::operator[](int i){
	...
	if(i>=0 && i<m_itemcount)
		...
		THROW_EXCP(excpArr,std::string("Section:") + m_sectname + " 的下标<<i<<越界");
	}
	return *l_it;
}
std::string &IniSection::operator[](const char *name)const{
	...
	if(i<m_itemcount){
		...
		THROW_EXCP(excpIniF, std::string("Section:") + m_sectname + " 下的Name:" + name + " 没有找到.");
	}
}
