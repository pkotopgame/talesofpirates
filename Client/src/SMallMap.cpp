...
void	CSMallMap2D::RenderScene()
{
	...
	else
	{
		...
		D3DXMatrixTranspose(&vpPose, &matrix);
	...
	for( m = 0; m < 3; m++)
	{
		for(int n = 0; n < 3; n++)
		{
			...
			{
				const D3DXVECTOR4 v[] = {
				{float(sx), float(sy),0, 1},
				{float(sx) + SHOWRSIZE ,float(sy),0,1},
				{float(sx) + SHOWRSIZE , float(sy) + SHOWRSIZE , 0, 1},
				{float(sx), float(sy) + SHOWRSIZE , 0, 1}
				};

				g_Render.SetVertexShaderConstant(9, &v[0], 1);
				g_Render.SetVertexShaderConstant(10, &v[1],1);
	
				g_Render.SetVertexShaderConstant(11, &v[2], 1);
				g_Render.SetVertexShaderConstant(12, &v[3], 1);
			}
#ifdef MGR
			_pVB->DrawSubset(0);
#else
